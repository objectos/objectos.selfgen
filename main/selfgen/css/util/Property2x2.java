/*
 * Copyright 2023 Objectos Software LTDA.
 *
 * Reprodução parcial ou total proibida.
 */
package selfgen.css.util;

import java.util.ArrayList;
import java.util.List;
import java.util.stream.Collectors;
import objectos.code.Code;
import objectos.selfgen.util.Check;

public final class Property2x2 extends PropertyClass {

  final String propertyName1;

  final String propertyName2;

  final List<Constant2> constants = new ArrayList<>();

  public Property2x2(String simpleName, String propertyName1, String propertyName2) {
    super(simpleName);

    this.propertyName1 = propertyName1;

    this.propertyName2 = propertyName2;
  }

  @Override
  public final void add(String name, String value1, String value2) {
    Check.notNull(name, "name == null");
    Check.notNull(value1, "value1 == null");
    Check.notNull(value2, "value2 == null");

    Constant2 cte;
    cte = new Constant2(name, value1, value2);

    constants.add(cte);
  }

  @Override
  final String generate(Code code, SelectorKind selector) {
    return code."""
    public enum \{simpleName} implements \{STYLE_CLASS} {
    \{generatePropertyConstants()}
      private final String className = \{CLASS_SEQ_ID}.next();

      private final String value1;

      private final String value2;

      private \{simpleName}(String value1, String value2) {
        this.value1 = value1;
        this.value2 = value2;
      }

      /**
       * Returns the CSS class name.
       *
       * @return the CSS class name
       */
      @Override
      public final String className() {
        return className;
      }

      /**
       * Returns the CSS style rule represented by this utility class.
       *
       * @return the CSS style rule
       */
      @Override
      public final String toString() {
        return \{selector.code} + " { \{propertyName1}: " + value1 + "; \{propertyName2}: " + value2 + " }";
      }

    }""";
  }

  @Override
  final String javadoc(Code code) {
    return code."""
    /**
     * Utility classes for the {@code \{propertyName1}} and {@code \{propertyName2}} CSS properties.
     */""";
  }

  private String generatePropertyConstants() {
    List<String> result;
    result = new ArrayList<>();

    for (var cte : constants) {
      result.add("  " + cte.name() + "(\"" +
          Code.escape(cte.value1()) + "\", \"" +
          Code.escape(cte.value2()) + "\")");
    }

    return result.stream().collect(Collectors.joining(",\n\n", "\n", ";\n"));
  }

}